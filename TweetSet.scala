...
abstract class TweetSet {
  ...
  def trending(words : List[String]) : TweetList = {
    filter(tweet => words.exists(word => tweet.text.contains(word))).descendingByRetweet
  }
}
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    if (right.isEmpty && left.isEmpty) {
      ...
      if (elem.retweets >= right.mostRetweeted.retweets && elem.retweets >= left.mostRetweeted.retweets) {
        elem
      }
  ...
  def descendingByRetweet: TweetList = {
    def list_tail (set: TweetSet, list: TweetList): TweetList = {
      if (set.isEmpty) {
        list
      }
      else {
        list_tail (set.remove (mostRetweeted), new Cons (this.mostRetweeted, list))
      }
    }
    list_tail (this.remove (mostRetweeted), new Cons (this.mostRetweeted, Nil))
  }
